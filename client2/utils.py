...
def load_data(data_path):
    """Load and preprocess the bank2 dataset."""
...
def preprocess_data(df):
    ...
    joblib.dump(preprocessor, './models/client2_preprocessor.pkl')
    
    return X_processed, y, preprocessor

def save_metrics(metrics, file_path='../dashboard/logs/client2_metrics.json'):
...
def send_model_to_server(model_path, server_url='http://localhost:5000'):
    ...
    try:
        ...
        response = requests.post(f"{server_url}/client2", files=files)
